var data = [{"id":1,"first_name":"Chadwick","last_name":"Ayre","email":"cayre0@cam.ac.uk","gender":"Male","img_src":"https://robohash.org/corporisquiaperiam.png?size=50x50&set=set1","class":11,"marks":18,"passing":false,"city":"Moorreesburg"},
...
{"id":4,"first_name":"Luigi","last_name":"Balcers","email":"lbalcers3@reverbnation.com","gender":"Agender","img_src":"https://robohash.org/eumaliquamest.png?size=50x50&set=set1","class":8,"marks":30,"passing":false,"city":"Nashtā Rūd"}, {"id":5,"first_name":"Adam","last_name":"Gurg","email":"agurg4@xrea.com","gender":"Male","img_src":"https://robohash.org/nemolaborumquis.png?size=50x50&set=set1","class":3,"marks":10,"passing":true,"city":"Sandviken"},
...
function printData(a) {
    for (let i = 0; i < a.length; i++) {
        ...
        row = table.insertRow();
        cell = row.insertCell();
        cell.textContent = a[i].id; 

        cell = row.insertCell();
        var img = document.createElement('img');
        img.src = a[i].img_src;
        cell.appendChild(img);
        let textNode = document.createTextNode( " " + a[i].first_name + " " + a[i].last_name);
        cell.appendChild(textNode);
        
        cell = row.insertCell();
        cell.textContent = a[i].gender;
        
        cell = row.insertCell();
        cell.textContent = a[i].class;
        
        cell = row.insertCell();
        cell.textContent = a[i].marks;
        
        cell = row.insertCell();
        cell.textContent = passing;
        
        cell = row.insertCell();
        cell.textContent = a[i].email;
    }
}
printData(data);

function submit() {
    const val = document.getElementById('inputEle');
    findValue(val.value);
}

function findValue(str) {
    let firstNamesMatch = data.filter(x => x.first_name.toLowerCase() == str.toLowerCase())
    let lastNamesMatch = data.filter(x => x.last_name.toLowerCase() == str.toLowerCase())
    let emailsMatch = data.filter(x => str.toLowerCase() === x.email.toLowerCase());

    table = document.getElementById("studentData");
    tr = table.getElementsByTagName("tr");
    for (let i = 0; i < tr.length; i++) {
        if(firstNamesMatch.length > 0) {
            td = tr[i].getElementsByTagName("td")[1];
        }
        if(lastNamesMatch.length > 0) {
            td = tr[i].getElementsByTagName("td")[1];
        }
...
function sortByGender() {
    ...
    printSortedData(males);
    printFemalesData(females);
}

function printFemalesData(females) {
    let table = document.createElement('table');
    table.classList.add('new-table');
    let headerRow = document.createElement('tr');
    let headers = ['ID', 'Name', 'Gender', 'Class', 'Marks', 'Passing', 'Email'];

    let myTable = document.getElementById('body');
    ...
    for (let i = 0; i < females.length; i++) {
        ...
        let passing = '';
        console.log(typeof females[i].passing);
        if(females[i].passing) {
            passing = 'Passing';
        }else {
            passing = 'Failing';
        }
        ...
        cell = row.insertCell();
        cell.textContent = females[i].id;

        cell = row.insertCell();
        var img = document.createElement('img');
        img.src = females[i].img_src;
        cell.appendChild(img);
        let textNode = document.createTextNode( " " + females[i].first_name + " " + females[i].last_name);
        cell.appendChild(textNode);

        cell = row.insertCell();
        cell.textContent = females[i].gender;
        
        cell = row.insertCell();
        cell.textContent = females[i].class;
        
        cell = row.insertCell();
        cell.textContent = females[i].marks;
        
        cell = row.insertCell();
        cell.textContent = passing;
        
        cell = row.insertCell();
        cell.textContent = females[i].email;
    }
    myTable.appendChild(table);
}

function printSortedData(arr) {
    function deleteRows() {
        var rowCount = studentData.rows.length;
        for (var i = rowCount - 1; i > 0; i--) {
            studentData.deleteRow(i);
        }
    }
    deleteRows();
    printData(arr);
